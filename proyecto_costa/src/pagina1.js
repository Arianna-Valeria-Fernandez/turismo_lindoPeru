...
const showDataFilms = (films) => {
  ...
  if(films.length === 0){
    const text = document.createTextNode("Lo siento pero la provincia que buscas no se encuntra en esta región :(");
    ...
    <section class='divison'>
      ...
      <section class='container__row'>
        <article class='container__text'>
            <h3 class='subtitle'>Kilómetros: </h3>
            <p class='paragraph'>${film.kilometros}</p>
        </article>
      </section>
      ...
      <section class='container__row'>
        <article class='container__text'>
          ...
          <p class='paragraph'> ${film.ubicacion}</p>
        </article>
        <article class='container__text'>
          <h3 class='subtitle'>Provincias: </h3>
          <p class='paragraph'> ${film.provincias}</p>
        </article>
      </section>
    </section>
...
const addFilmPeople = (people) => {
  ...
  containerPeople.innerHTML = `
    <section class="secondaryTitle">
      <h2 class="subtitle">Provincias</h2>
    </section>
  `;
  // Menu Characters Bar
  ...
  <option value="all" disabled selected>Tipo</option>
...
const addFilmSection = (data, containerFilmInformation, title) => {
  containerFilmInformation.innerHTML = `
        <section class="container__subtitle">
            ...
            <p class="showing">Mostrando ${data.length}</p>
        </section>
  `;
